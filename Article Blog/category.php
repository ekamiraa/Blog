      ...
      <section>
        <div class="container">
          <div class="row">
            ...
            $query_count = "SELECT COUNT(*) as total FROM article WHERE id_kategori = $id_kategori";
            $result_count = mysqli_query($conn, $query_count);
            $row_count = mysqli_fetch_assoc($result_count);
            $total_articles = $row_count['total']; // Total artikel dalam kategori tersebut

            $total_pages = ceil($total_articles / 4); // Jumlah total halaman

            $offset = ($page - 1) * 4; // Offset untuk query

            $query = "SELECT * FROM article 
              INNER JOIN user ON user.id_user = article.id_user
              INNER JOIN kategori ON kategori.id_kategori = article.id_kategori 
              WHERE article.id_kategori = $id_kategori
              ORDER BY id_article DESC
              LIMIT 4 OFFSET $offset";
                $result = mysqli_query($conn, $query);        
            ?>
            <div class="col-md-9" data-aos="fade-up">
              <h3 class="category-title">Category: <?php echo $kategori; ?></h3>
            <?php
            ...
            ?>
              <div class="d-md-flex post-entry-2 half">
                <a href="single-post.php?id_article=<?php echo $data['id_article']; ?>" class="me-4 thumbnail">
                  <img
                    src="<?php echo $imagePath; ?>"
                    alt=""
                    class="img-fluid custome-image"
                  />
                  <style>
                        .custome-image {
                            width: 700px; /* Ubah nilai ini sesuai kebutuhan Anda */
                            height: 300px;
                            object-fit: cover;
                        }
                    </style>
              ...
              <div class="text-start py-4">
                <div class="custom-pagination">
                  ...
                  if ($page > 1) {
                      $prev_page = $page - 1;
                      echo '<a href="?id_kategori=' . $id_kategori . '&page=' . $prev_page . '" class="prev">Previous</a>';
                  }

                  // Tombol Halaman
                  for ($i = 1; $i <= $total_pages; $i++) {
                      echo '<a href="?id_kategori=' . $id_kategori . '&page=' . $i . '"';
                      if ($i == $page) {
                          echo ' class="active"';
                      }
                      echo '>' . $i . '</a>';
                  ...
                  if ($page < $total_pages) {
                      $next_page = $page + 1;
                      echo '<a href="?id_kategori=' . $id_kategori . '&page=' . $next_page . '" class="next">Next</a>';
                  }
                  ?>
                </div>
              </div>
